...
%                      fprintf('n=%d, x:%d - y:%d\n     q:%d - t:%d\n',n,x,y,q,t);
        ...
        function curve = planTrajectory(x,y,th,sgn)
            ...
            q = atan2(y,x);
            % set pi() equal to - pi() to avoid out of bounds in scalar
            % field.
            ...
            oneBad = false;
            twoBad = false;
            if(isinf(a1) || isinf(b1)) % makes sure there exist a solution in space 1
                oneBad = true;
            elseif(isinf(a2) || isinf(b2))% makes sure there exist a solution in space 2
                twoBad = true;
            end;
            ...
            if(oneBad==true && twoBad==true)
                ...
                return;
            elseif(oneBad==true) %
                au=a2;bu=b2;     % If there is no choice, pick the valid solution
            elseif(twoBad==true) %
                au=a1;bu=b1;     
            elseif(r2 > r1)      %
                au=a1;bu=b1;     % Else pick the one with lowest energy, i.e. the most direct.
            else                 %
                au=a2;bu=b2;     
            end
    ...
    methods(Access = private)
       ...
       function setParms(obj,parms)
        ...
        function integrateCommands(obj)
            ...
            obj.distArray(1) = 0.0;
            obj.poseArray(1,1) = 0.0;%x
            obj.poseArray(2,1) = 0.0;%y
            obj.poseArray(3,1) = 0.0;%th
            obj.curvArray(1) = 0.0;
            ...
            for i=1:obj.numSamples-1
                ...
                obj.curvArray(i+1) = obj.distArray(i+1)*(a+b*obj.distArray(i+1))*(obj.distArray(i+1)-sf);
                
                obj.poseArray(3,i+1) = obj.poseArray(3,i) + obj.curvArray(i+1)*ds;
                obj.poseArray(1,i+1) = obj.poseArray(1,i) + cos(obj.poseArray(3,i+1))*ds;
                obj.poseArray(2,i+1) = obj.poseArray(2,i) + sin(obj.poseArray(3,i+1))*ds;
            end
            i = obj.numSamples;
            s = (i-1)*ds;  
            obj.distArray(i) = s;
